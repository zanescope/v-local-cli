...
2. 下载候选件，按 `references/macos-acceptance.md` 和支持平台清单完成真机验收。
3. 从已验收提交创建并推送与包版本一致的标签，例如 `v0.1.0-dev.1`。
4. `Signed release` 会重新构建，完成 Windows Authenticode、macOS Developer ID/notarization、摘要与来源证明，然后创建 GitHub Release。
