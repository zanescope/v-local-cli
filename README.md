...
单个 Go 二进制即可完成账号发现、密钥验证、只读快照、联系人/聊天/搜索、语音转写与 OCR 读取、朋友圈与公众号查询、图片解密与导出。
...
- **联系人/聊天/搜索** — 联系人与群、聊天历史（名片、小程序、视频号、红包、引用、合并记录等解析为紧凑摘要）、结构化文本搜索。
...
| 会话 | `contacts` · `history` · `search` · `stats` |
...
成功结果写入 stdout；失败写入 stderr，并以非零退出码结束。所有 JSON 都带顶层 `schema_version`：
