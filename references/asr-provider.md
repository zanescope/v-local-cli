...
项目提供独立的 `v-local-cli-sensevoice` 仓库实现：它使用 sherpa-onnx 的 Go 绑定读取本地 SenseVoice int8 模型，不改变主仓依赖边界。该实现已在 Windows amd64 上通过协议测试、构建测试，并用一条真实微信语音完成非空离线转写；真实文本、会话和联系人信息不进入测试记录。
...
```json
{"protocol":"v-local-cli-asr/1","transcript":"识别文字","engine":"sensevoice","model":"sensevoice-small-int8","language":"zh","network_used":false}
```

约束：

- `source_audio_sha256` 是原始语音负载（SILK 解码前）的摘要，仅作溯源标记，用于把转写结果关联回源消息。它**不是**输入 WAV 的摘要，适配器只拿到解码后的 WAV，因此结构上无法、也不应据此做完整性校验；适配器应将其视为不透明标识原样忽略。
